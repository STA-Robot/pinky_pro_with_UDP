...
## 패킷 명세 
```
본 노드는 UDP를 통해 데이터를 전송하며, 각 패킷은 헤더(Header)와 이미지 데이터(Payload)로 구성됩니다.
헤더 (Header) /데이터 형식: String (UTF-8) /크기: 가변  /설명: robot_name + `
데이터 (Payload)Binary /데이터 형식:(JPEG) /크기:가변 /설명: OpenCV imencode로 압축된 이미지 바이트 데이터

참고: 서버 측에서 데이터를 파싱할 때 | (파이프 문자, ASCII 124)를 기준으로 앞부분은 로봇 이름으로, 뒷부분은 이미지 바이너리 데이터로 분리하여 처리하십시오.
```